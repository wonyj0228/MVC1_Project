...
<%
    ...
    String id = request.getParameter("id");  //Login.jsp에서 입력된 id 변수값으로 넣기
    String pw = request.getParameter("pw");  //Login.jsp에서 입력된 pw 변수값으로 넣기
    
    userDB db = new userDB();  //데이터베이스 처리할 도구 가져오기
    boolean result = db.check(id, pw);  //check 메서드 사용해서 id,pw가 일치한지 확인
    
    if(result){  //id,pw 일치하여 result 값이 true일때
...
<html>
	...
	<body>
		...
		<nav id="Content" align="center"> <!-- 각 조원들이 넣게될 본문 레이아웃 생성 -->
			...
			<% if(session.getAttribute("id") == null) { %>   <!-- 세션값이 없을 때 -->
				<script type ="text/javascript">  /* 자바 스크립트 실행  */
					alert("로그인에 실패했습니다. 아이디와 비밀번호를 다시 입력해주세요.")   /* 로그인 실패 알림 띄우기  */
				</script> <!-- 자바스크립트 종료 -->
				
				 <form action = "LoginConnect.jsp">   <!-- 결과값을 다시 LoginConnect로 보내주는 login창을 보여줌 -->
